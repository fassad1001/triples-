                                         ...
                                         , const int classInstancesCount)
{
    ...
    while(canGenerate)
    {
        ...
        if(classes.count() > 0)
        {
            ...
            addClass(className, classes.toList().at(qrand() % allClasses().count()));
            classes += className;
        }
        //иначе
        else
...
//        {
//        }
//        while(instancesSummary.count() < instancesCountSummary)
//        {
...
//        }
        ...
        if(classes.count() >= ClassCountSummary)
        {
            //прервать
            canGenerate = false;
        }
